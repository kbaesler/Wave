...
namespace Wave.Searchability.Services
{
        ...
        where TSearchableRequest : TextSearchServiceRequest, new()
    {
        ...
        protected override void Find(TSearchableRequest request, IMap data, CancellationToken token)
        {
            ...
            this.Find(request, data, layers, tables, token);
        }
        ...
        protected void Find(TSearchableRequest request, IMap map, List<IFeatureLayer> layers, List<ITable> tables, CancellationToken token)
        {
            if (request == null)
                throw new ArgumentNullException("request");

            this.SearchLayers(request, map, layers, token);
            this.SearchTables(request, map, tables, layers, token);
        }
        ...
        private void Attach(IObjectClass searchClass, ISet rows, IEnumerable<SearchableRelationship> relationships, IFeatureLayer layer, List<IFeatureLayer> layers, TSearchableRequest request, IMap map, CancellationToken cancellationToken)
        {
            ...
            foreach (var item in relationships)
            {
                ...
                foreach (var relationshipClass in relationshipClasses.AsEnumerable())
                {
                    ...
                    if (item.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase)
                        || item.Name.Equals(SearchableRelationship.Any.Name))
                    {
                        ...
                        if (isFeatureClass || isRecursive)
                        {
                            ...
                            if (isRecursive)
                            {
                                this.Attach(relClass, set, item.Relationships, layer, layers, request, map, cancellationToken);
                            }
                            else
                            {
                                ...
                                if (layer != null)
                                {
                                    foreach (var row in set.AsEnumerable<IFeature>())
                                    {
                                        this.Add(row, layer, request, map, cancellationToken);
                                    }
                                }
                            }
                        }
        ...
        /// <param name="map">The map.</param>
        /// <param name="cancellationToken">The token.</param>
        private void SearchLayer(IFeatureLayer layer, SearchableLayer item, TSearchableRequest request, IMap map, CancellationToken cancellationToken)
        {
            if (cancellationToken.IsCancellationRequested)
                return;
            ...
            foreach (var feature in searchClass.Fetch(filter))
            {
                ...
                this.Add(feature, layer, request, map, cancellationToken);
            }
        }
        ...
        /// <param name="layers">The layers.</param>
        /// <param name="cancellationToken">The token.</param>
        private void SearchLayers(TSearchableRequest request, IMap map, List<IFeatureLayer> layers, CancellationToken cancellationToken)
        {
            var items = request.Packages.SelectMany(inventory => inventory.Items.OfType<SearchableLayer>()).ToList();
            foreach (var i in items)
            {
                ...
                foreach (var l in layers.Where(o => o.Name.Equals(item.Name, StringComparison.CurrentCultureIgnoreCase) || (item.NameAsClassModelName && o.FeatureClass.IsAssignedClassModelName(item.Name))))
                {
                    ...
                    this.SearchLayer(layer, item, request, map, cancellationToken);

                    this.TraverseRelationships(layer.FeatureClass, layer, layers, item, request, map, cancellationToken);
                }
            }
        }
        ...
        /// <param name="map">The map.</param>
        /// <param name="cancellationToken">The token.</param>
        private void SearchRelationship(IObjectClass searchClass, IFeatureLayer layer, List<IFeatureLayer> layers, SearchableItem item, SearchableRelationship relationship, TSearchableRequest request, IMap map, CancellationToken cancellationToken)
        {
            if (cancellationToken.IsCancellationRequested)
                return;
            ...
            if (rowsAffected > 0)
            {
                this.Attach(searchClass, rows, item.Relationships, layer, layers, request, map, cancellationToken);
            }
        }
        ...
        /// <param name="map">The map.</param>
        /// <param name="token">The token.</param>
        private void SearchTable(ITable table, SearchableItem item, List<IFeatureLayer> layers, TSearchableRequest request, IMap map, CancellationToken token)
        {
            ...
            if (rowsAffected > 0)
            {
                ...
                {
                    item.Relationships.Add(SearchableRelationship.Any);
                }

                IObjectClass searchClass = (IObjectClass) table;
                this.Attach(searchClass, rows, item.Relationships, null, layers, request, map, token);
            }
        }
        ...
        /// <param name="layers">The layers.</param>
        /// <param name="cancellationToken">The token.</param>
        private void SearchTables(TSearchableRequest request, IMap map, List<ITable> tables, List<IFeatureLayer> layers, CancellationToken cancellationToken)
        {
            var items = request.Packages.SelectMany(inventory => inventory.Items.OfType<SearchableTable>()).ToList();
            foreach (var i in items)
            {
                var item = i;
                ...
                foreach (var t in tables.Where(o => ((IDataset) o).Name.Equals(item.Name, StringComparison.CurrentCultureIgnoreCase) || (item.NameAsClassModelName && o.IsAssignedClassModelName(item.Name))))
                {
                    ...
                    this.SearchTable(table, item, layers, request, map, cancellationToken);

                    this.TraverseRelationships((IObjectClass) table, null, layers, item, request, map, cancellationToken);
                }
            }
        }
        ...
        /// <param name="map">The map.</param>
        /// <param name="cancellationToken">The token.</param>
        private void TraverseRelationships(IObjectClass itemClass, IFeatureLayer layer, List<IFeatureLayer> layers, SearchableItem item, TSearchableRequest request, IMap map, CancellationToken cancellationToken)
        {
            if (cancellationToken.IsCancellationRequested)
                return;
            ...
            foreach (var r in item.Relationships)
            {
                ...
                foreach (var relationshipClass in relationshipClasses.AsEnumerable())
                {
                    ...
                    if (relationship.Name.Equals(name, StringComparison.InvariantCultureIgnoreCase) || relationship.Name.Equals(SearchableRelationship.Any.Name))
                    {
                        ...
                        this.SearchRelationship(searchClass, layer, layers, item, relationship, request, map, cancellationToken);

                        this.TraverseRelationships(searchClass, layer, layers, relationship, request, map, cancellationToken);
                    }
                }
            }
        }
